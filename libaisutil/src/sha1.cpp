...
namespace {
    ...
    */
   static void SHA1init(context_type &context)
     {
    ...
    * 07/04/2002 pickle - Cleaned up a bit, made it a little safer..
    */
   static void SHA1update(context_type &context, unsigned char *data,
			  unsigned int len)
     {
	unsigned long i, j;
    ...
    * 07/04/2002 pickle - Cleaned up a bit, made it a little safer..
    */
   static void SHA1final(unsigned char digest[20], context_type &context)
     {
	unsigned long i, j;
...
/* SHA1_Digest - Construct a blank digest structure
 * Original 22/07/2003 pickle
 */
SHA1_Digest::SHA1_Digest(void)
{
   // Fill ourself with zeros
 ...
 */
const std::string SHA1_Digest::toStr(const unsigned char base,
				     const std::string::size_type pad)
{
   std::string output;

   for (unsigned char i = 0; i < 5; ++i) {
      output += Utils::baseXStr(u_long[i], base, true).prepad(pad, '0');
   }
